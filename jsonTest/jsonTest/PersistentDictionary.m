	...
	if (self = [super init]) {
		...
		if ([fileManager fileExistsAtPath:_filePath]) {
			...
			if (propListData) {
				...
				_dictionary = [NSPropertyListSerialization propertyListFromData:propListData mutabilityOption:NSPropertyListMutableContainersAndLeaves format:&format errorDescription:nil];
			}
		...
		if (!_dictionary) {
			...
			if ([fileManager fileExistsAtPath:defaultDicPath]) {
				...
				if (jsonData) {
					...
					if (error) {
						//NSLog(HELPER, ERR, @"JSON deserialization error for dictionary [%@]: %@", _fileName, error);
...
- (void) deleteDictionary {
	...
	if ([fileManager fileExistsAtPath:_filePath]) {
        ...
        [fileManager removeItemAtPath:_md5Path error:nil];
	}
